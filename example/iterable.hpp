...
#ifndef __INTERFACE_____EXAMPLE_ITERABLE_I
#define __INTERFACE_____EXAMPLE_ITERABLE_I
//object that can get size of it
class HasSize {
 private:
  template<typename __T>
  struct __VirtualTable {
    size_t(__T::* size)()const;
    inline static __VirtualTable virtualTable{&__T::size,};
  };
 public:
  struct Dummy{};
 private:
  ...
  Proxy<Dummy> object;
 public:
  HasSize(void* vtb, Proxy<Dummy>&& obj) : virtualTable(reinterpret_cast<__VirtualTable<Dummy>*>(vtb)), object(std::move(obj)) {}
  template<typename __U, typename E = std::enable_if_t<!is_shared_ptr_v<__U> && !is_unique_ptr_v<__U>>>HasSize(__U* u) : virtualTable(reinterpret_cast<__VirtualTable<Dummy>*>(&__VirtualTable<__U>::virtualTable)), object(reinterpret_cast<Dummy*>(u)) {}
  template<typename __U, typename E = std::enable_if_t<!is_shared_ptr_v<__U> && !is_unique_ptr_v<__U>>>HasSize(__U& u) : virtualTable(reinterpret_cast<__VirtualTable<Dummy>*>(&__VirtualTable<__U>::virtualTable)), object(reinterpret_cast<Dummy*>(&u)) {}
  template<typename __U, typename E = std::enable_if_t<!is_shared_ptr_v<__U> && !is_unique_ptr_v<__U> && !std::is_lvalue_reference_v<__U>>>HasSize(__U&& u) : virtualTable(reinterpret_cast<__VirtualTable<Dummy>*>(&__VirtualTable<__U>::virtualTable)), object(std::forward<__U>(u)) {}
  template<typename __U>HasSize(const std::shared_ptr<__U>& u) : virtualTable(reinterpret_cast<__VirtualTable<Dummy>*>(&__VirtualTable<__U>::virtualTable)), object(u) {}
  template<typename __U>HasSize(std::shared_ptr<__U>&& u) : virtualTable(reinterpret_cast<__VirtualTable<Dummy>*>(&__VirtualTable<__U>::virtualTable)), object(std::move(u)) {}
  template<typename __U>HasSize(const std::unique_ptr<__U>& u) : virtualTable(reinterpret_cast<__VirtualTable<Dummy>*>(&__VirtualTable<__U>::virtualTable)), object(u) {}
  template<typename __U>HasSize(std::unique_ptr<__U>&& u) : virtualTable(reinterpret_cast<__VirtualTable<Dummy>*>(&__VirtualTable<__U>::virtualTable)), object(std::move(u)) {}
  size_t size() {
    return ((object.get())->*(virtualTable->size))();
  }
};
#endif
